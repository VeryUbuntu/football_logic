...
### 2. Clone & Build
**Crucial Step**: You must provide your Supabase credentials during the build process so they are baked into the client-side code.
...
sudo docker build \
  --build-arg NEXT_PUBLIC_SUPABASE_URL="your_real_url_here" \
  --build-arg NEXT_PUBLIC_SUPABASE_ANON_KEY="your_real_key_here" \
  -t football-logic .
```
